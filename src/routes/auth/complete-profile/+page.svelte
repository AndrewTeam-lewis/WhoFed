<script lang="ts">
  ...
  async function handleSubmit(e: Event) {
    ...
    if (!formData.firstName) {
      error = $t.auth.error_display_name_required;
      return;
    }
...
<svelte:head>
  <title>{$t.auth.complete_profile_title} - WhoFed</title>
</svelte:head>
...
<div class="min-h-screen bg-gray-50 flex items-center justify-center p-4 md:p-6">
  <div class="w-full max-w-md bg-white rounded-2xl shadow-sm p-6 md:p-8">
    ...
    <h1 class="text-2xl md:text-3xl font-bold text-gray-800 text-center mb-2">{$t.auth.complete_profile_title}</h1>
    <p class="text-sm text-gray-500 text-center mb-6 md:mb-8">{$t.auth.complete_profile_subtitle}</p>
    ...
    <form on:submit={handleSubmit} class="space-y-4">
      <div class="mb-4">
        <label for="firstName" class="block text-sm font-medium text-gray-800 mb-2">{$t.auth.complete_profile_display_name_label}</label>
        <input
          id="firstName"
          type="text"
      ...
      >
        {#if loading}
          ...
          <span class="ml-2">{$t.auth.complete_profile_creating}</span>
        {:else}
          {$t.auth.complete_profile_button}
        {/if}
      </button>
    </form>
  </div>
