    ...
    <div class="bg-white rounded-[32px] p-8 w-full max-w-sm shadow-xl text-center">
        {#if loading}
            <div class="py-12 flex flex-col items-center">
                 ...
                 <p class="text-gray-500 font-bold">{$t.join.loading}</p>
            </div>
        {:else if error}
            <div class="py-6">
                <!-- Error UI -->
                ...
                <h2 class="text-xl font-bold text-gray-900 mb-2">{$t.join.oops}</h2>
                <p class="text-gray-500 mb-6">{error}</p>
                <button on:click={() => goto('/')} class="text-brand-sage font-bold hover:underline">{$t.join.go_home}</button>
            </div>
        {:else if isGuest && showAppDownload}
            ...
            <div class="py-6">
                ...
                <h1 class="text-2xl font-bold text-gray-900 mb-2">{$t.join.download_title}</h1>
                <p class="text-gray-500 mb-2">
                    {$t.join.download_desc}
                </p>
                <p class="text-sm text-gray-400 mb-8">
                    {$t.join.download_note}
                </p>
                ...
                <div class="space-y-3 mb-6">
                    ...
                    >
                        ...
                        <span>{$t.join.download_app_store}</span>
                    </a>
                    ...
                    >
                        ...
                        <span>{$t.join.download_google_play}</span>
                    </a>
                </div>

                <div class="pt-6 border-t border-gray-100">
                    <p class="text-xs text-gray-500 mb-3 text-center">{$t.join.or_continue_web}</p>
                    <div class="space-y-2">
                        <button
        ...
        {:else if isGuest}
            ...
            <div class="py-12 flex flex-col items-center">
                <div class="animate-spin h-10 w-10 border-4 border-brand-sage rounded-full border-t-transparent mb-4"></div>
                <p class="text-gray-500 font-bold">{$t.join.opening_app}</p>
            </div>
        {:else if emailMismatch}
            ...
            <div class="py-6">
                ...
                <h2 class="text-xl font-bold text-gray-900 mb-2">{$t.join.wrong_account}</h2>
                <p class="text-gray-500 mb-2">
                    {$t.join.invite_sent_to}
                </p>
        ...
        {:else}
            ...
            <div class="py-6">
                ...
                <h1 class="text-2xl font-bold text-gray-900 mb-2">{$t.join.join_title}</h1>
                <p class="text-gray-500 mb-2">
                    {$t.join.invite_desc.replace('{owner}', ownerName).replace('{count}', memberCount)}
                </p>

                <!-- Show which account they're joining as -->
                <p class="text-xs text-gray-400 mb-8">
                    {$t.join.joining_as} <span class="font-semibold">{currentUser?.email}</span>
                    &middot;
                    <button on:click={switchAccount} class="text-brand-sage font-semibold hover:underline">{$t.join.switch}</button>
                </p>
                ...
                <button on:click={() => goto('/')} class="mt-6 text-sm text-gray-400 font-bold hover:text-gray-600">{$t.common.cancel}</button>
            </div>
        {/if}
    </div>
</div>
