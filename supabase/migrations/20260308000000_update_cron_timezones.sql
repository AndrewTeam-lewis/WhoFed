-- Migration: Update notifications cron to use household timezone
...
declare
    ...
    v_label_lower text;
begin
    -- 1. Loop through all active schedules joined with pets AND households
    for v_schedule in
        select
            s.id as schedule_id,
            s.label,
    ...
    loop
        -- 2. Calculate current time in the HOUSEHOLD'S timezone
        v_pet_tz := v_schedule.hh_timezone;
    ...
    -- 2. ONE-TIME TASKS (Updated to use household timezone)
    for v_schedule in
        select
            t.id as task_id,
            t.label,
            ...
            p.name as pet_name,
            hm.user_id,
            to_char(t.due_at at time zone coalesce(h.timezone, 'UTC'), 'HH24:MI') as task_time_str,
            pr.language
        from daily_tasks t
